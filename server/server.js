...
function createPlayer(id) {
  return {
    ...
    facing: 1, mouseX: 0, mouseY: 0, onGround: false, invincible: 0,
    slots: [null, null, null, null],
    returning: false, returnTimer: 0, inBush: false
  };
}
...
function resolveBlockCollision(p) {
  ...
  for (let c = c1; c <= c2; c++) {
    for (let r = r1; r <= r2; r++) {
      const b = blockAt(c, r);
      if (!b) continue;
      if (b.type === 4) { p.inBush = true; continue; }
      if (b.type !== 1) continue;
      const bx = b.c * BLOCK_SIZE;
      const by = b.r * BLOCK_SIZE;
      const oL = (p.x + p.width) - bx;
      const oR = (bx + BLOCK_SIZE) - p.x;
      const oT = (p.y + p.height) - by;
      const oB = (by + BLOCK_SIZE) - p.y;
      const minO = Math.min(oL, oR, oT, oB);
      if (minO === oT && p.vy >= 0) { p.y = by - p.height; p.vy = 0; p.onGround = true; }
      else if (minO === oB && p.vy < 0) { p.y = by + BLOCK_SIZE; p.vy = 0; }
      else if (minO === oL && p.vx > 0) { p.x = bx - p.width; p.vx = 0; }
      else if (minO === oR && p.vx < 0) { p.x = bx + BLOCK_SIZE; p.vx = 0; }
    }
  }
}
...
function updateHook(p) {
  ...
  if (!p.hook.attached) {
    ...
    if (hit) { p.hook.attached = true; p.hook.x = hit.x; p.hook.y = hit.y; }
    if (p.hook.x < 0 || p.hook.x > GAME_WIDTH || p.hook.y < 0 || p.hook.y > GAME_HEIGHT) p.hook.active = false;
  } else {
    const dx = p.hook.x - sx;
    const dy = p.hook.y - sy;
    const dist = Math.sqrt(dx*dx + dy*dy);
    if (dist > 15) { p.vx += dx * 0.025; p.vy += dy * 0.025; p.state = 'hooked'; }
    else { p.hook.active = false; p.state = 'normal'; }
  }
}
...
function updateBullets() {
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    if (b.life <= 0 || b.x < 0 || b.x > GAME_WIDTH || b.y < 0 || b.y > GAME_HEIGHT) bullets.splice(i, 1);
  }
}
...
function updatePhysics() {
  for (const id in players) {
    ...
    if (!p.onGround && p.state !== 'hooked' && p.state !== 'hand_mode') p.vx *= 0.98;
    else if (p.state === 'normal') p.vx *= 0.85;
    updateHook(p);
    updateHand(p);
  }
  updateMovables();
...
io.on('connection', (socket) => {
  ...
  socket.on('input', (data) => {
    ...
    if (p.state === 'hand_mode' && p.hand.attached) {
      ...
      // 休憩所境界より右（x < REST_ZONE.x）なら射撃可
      if (data.attack && p.x < REST_ZONE.x) {
        const angle = Math.atan2(p.mouseY - (p.y + p.height/2), p.mouseX - (p.x + p.width/2));
        bullets.push({ x: p.x + p.width/2, y: p.y + p.height/2, vx: Math.cos(angle)*BULLET_SPEED, vy: Math.sin(angle)*BULLET_SPEED, owner: p.id, life: BULLET_LIFE, color: p.color });
      }
      return;
    }
    ...
    if (data.hook) {
      if (p.hook.active) { p.hook.active = false; p.state = 'normal'; }
        ...
        const angle = Math.atan2(p.mouseY - (p.y + p.height/2), p.mouseX - (p.x + p.width/2));
        p.hook.active = true; p.hook.attached = false; p.hook.len = 0; p.hook.angle = angle * 180 / Math.PI;
      }
    }
    ...
    if (data.pasta && p.state !== 'hand_mode') {
      if (p.hand.active && !p.hand.attached) { p.hand.active = false; }
        ...
        const angle = Math.atan2(p.mouseY - (p.y + p.height/2), p.mouseX - (p.x + p.width/2));
        p.hand.active = true; p.hand.attached = false; p.hand.len = 0; p.hand.angle = angle * 180 / Math.PI;
      }
    }
    ...
    if (data.rest && !p.returning && p.zone !== 'rest') {
      p.returning = true; p.returnTimer = 180;
      p.vx = 0; p.vy = 0;
    }
  });
